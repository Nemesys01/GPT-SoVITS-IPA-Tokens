...
def clean_text(text, language, version=None):
    print(f"##> clean_text: language={language}")
    if version is None:
    ...
    if version == "v1":
        ...
        language_module_map = {"ipa": "raw_ipa", "zh": "chinese2", "ja": "japanese", "en": "english", "ko": "korean", "yue": "cantonese"}
    ...
    if hasattr(language_module, "text_normalize"):
        ...
        print(f"Post Normalize: {norm_text}")
    else:
        print(f"##> No text normalization occured. Norm Text is: {text}")
        norm_text = text

    print("### Grapheme to Phoneme processing...")
    if language == "zh" or language == "yue":  ##########
        ...
        print("##> Language is IPA, mimicking english for now...")
        language_module = __import__("text.english", fromlist=[language_module_map[language]])
        phones = language_module.g2p(norm_text)
        word2ph = None
    else:
        phones = language_module.g2p(norm_text)
